...
public class 데이터관리 extends JFrame implements ActionListener{
	...
	public 데이터관리()
	{
		...
		for(int i=0;i<titles.length;i++) {
			String[] datas= {mnos[i],titles[i],actors[i],genres[i],grades[i]};
		model.addRow(datas);
		}
		setSize(800, 600);
		setVisible(true);
	...
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		if(e.getSource()==btn || e.getSource()==tf) // btn 버튼을 눌렀을 때
		{
			String cate=(String)box.getSelectedItem();
			// System.out.println(cate); => 테스트 코드
			// 입력값 읽기
