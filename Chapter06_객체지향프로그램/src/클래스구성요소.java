 ...
 *  		 ex) 사용자 정수 2개 => +한 값 요청 => 정수1개 리턴
 *  		     - 리턴은 반드시 한개만 전송 가능
 *   			 - 여러개는 배열/클래스
 *   			 - 일반데이터형 사용 가능 (참조형 : String, 배열)
 *  - 메소드명 : 소문자로 시작
 *    	  	  변수 => 식별자 동일
 *  - 매개변수 : 사용자가 요청한 값
 *  
 *  [접근지정어] public / private / protected / default
 *  [제어어] default / static / abstract / final
 *                   ------   --------   -----
 *               컴파일시 자동저장/추상메도스:선언/종단(수정 불가능)
 *         => static : 공통으로 사용
 *         => static이 없는 경우에는 객체 사용(각자)
 *            
 *  2) 한가지 기능이 어디까지인지
 *     
